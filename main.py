...
parser = argparse.ArgumentParser(description='PyTorch InterMOT Training')
parser.add_argument('--config', default='./experiments/config_train.yaml')
...
def main():
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=config.lr_scheduler.base_lr, betas=(0.9, 0.99), 
...
def train(train_loader, val_loader, tracking_module,
    ...
    for i, (input, det_info, det_id, det_cls,
        ...
        if lr_scheduler is not None and (curr_step + 1) % config.val_freq == 0:
            
            lr_scheduler.step()
            current_lr = lr_scheduler.get_lr()
            logger.info("current_lr:{}".format(current_lr))
